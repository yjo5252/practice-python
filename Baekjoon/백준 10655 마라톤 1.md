...
i번째 줄의 첫 번째 정수는 체크포인트 i의 x좌표, 두 번째 정수는 y좌표이다. (-1000 ≤ x, y ≤ 1000)

체크 포인트의 좌표는 겹칠 수도 있다 - 젖소 박승원은 체크포인트를 건너뛸 때 그 번호의 체크포인트만 건너뛰며, 
...
if __name__ == '__main__':
		...
		for i in range(1, N-1):
				...
				cur_node = v[i]
				next_node = v[i+1]
				dist = abs(prev_node[0]-cur_node[0]) + abs(prev_node[1]-cur_node[1])+ abs(cur_node[0]-next_node[0]) + abs(cur_node[1] - next_node[1])
				straight = abs(prev_node[0]-next_node[0]) + abs(prev_node[1] - next_node[1])
